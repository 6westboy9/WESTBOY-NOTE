...
note right of CFG
Claude Code 不会自动知道数据库地址和密码
这些通常由 MCP Server 配置提供
end note

U -> CC : 启动 Claude Code / 载入项目
CC -> CFG : 读取 MCP 配置
CFG --> CC : 返回 mysql server 配置

CC -> MCP : 按配置启动本地 MCP Server\n(常见为 stdio)
note over CC,MCP
MCP 是 host-client-server 架构；
Claude Code 作为 host，建立到该 server 的 client 连接
end note

MCP -> DB : 使用 env 中的连接信息尝试连接数据库
DB --> MCP : 连接成功 / 失败

MCP --> CC : 注册成功，暴露可用 tools
note over MCP,CC
例如：
list_tables
describe_table
